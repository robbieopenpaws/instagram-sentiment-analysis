...
// Inline Legend Component (for each post)
const InlineLegend = () => (
...
function App() {
  ...
  const [analysisProgress, setAnalysisProgress] = useState({ current: 0, total: 0, currentPost: '' });
  ...
  const runAccountWideAnalysis = async () => {
    ...
    try {
      ...
      setAnalysisProgress({ current: 0, total: highEngagementPosts.length, currentPost: '' });
      
      let allAccountComments = [];
      let totalPostsAnalyzed = 0;
      
      for (let i = 0; i < highEngagementPosts.length; i++) {
        const post = highEngagementPosts[i];
        const postDate = new Date(post.timestamp).toLocaleDateString();
        
        setAnalysisProgress({ 
          current: i + 1, 
          total: highEngagementPosts.length, 
      ...
      setAnalysisProgress({ current: 0, total: 0, currentPost: '' });
    }
  };
      ...
      {runningAccountAnalysis && analysisProgress.total > 0 && (
        <div className="analysis-progress">
          <ProgressBar 
            ...
            label="Account-Wide Analysis Progress"
          />
